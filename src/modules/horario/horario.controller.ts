...
/* =========================
   HELPERS
========================= */
// const normalizarFecha = (d = new Date()) => {
//   const f = new Date(d);
//   f.setHours(0, 0, 0, 0);
//   return f;
// };
...
let registro = await RegistroHorario.findOne({
  ...
  usuario: req.user.id,
  fecha: fechaLocal,
  fichajes: [{ tipo: "ENTRADA", hora: ahora }],
  minutosTrabajados: 0,
});
...
// 👉 AQUÍ VA EL BLOQUE QUE PREGUNTAS
...
/* ⛔ Anti doble clic (30s) */
if (
  ahora.getTime() - new Date(ultimo.hora).getTime() < 30_000
) {
  return res.status(400).json({
   ...
   /* 🔥 SI ES SALIDA → EXPULSAR CRM EN TODAS LAS SESIONES */
  ...
  dias.push({
    ...
    fichajes: registro?.fichajes
      ? registro.fichajes
          ...
          .map((f: any) => ({
            tipo: f.tipo,
            hora: new Date(f.hora).toLocaleTimeString("es-ES", {
              hour: "2-digit",
              minute: "2-digit",
            }),
          }))
